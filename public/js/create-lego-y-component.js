...
AFRAME.registerComponent('create-lego-y-component', {
    ...
    createLego1 : function (){
        ...
        //legoElen.setAttribute('obj-model','obj:assets/models/lego.obj;');
        lego1Elem.setAttribute('material', 'color', 'yellow');  //set material/texture

        //random tranform
        lego1Elem.setAttribute('position', {x:(Math.random() * 6.0) - 3.0, y:0, z:-4.0 - (Math.random() * 3.0)}); //random x around axis and random z behind button
        const randScale = 0.2 + (Math.random() * 0.8);
        lego1Elem.setAttribute('scale', {x:0.5, y:0.5, z:0.5});  //scale
        lego1Elem.setAttribute('rotation', {x:0, y:0, z:0});   //random y rotation
